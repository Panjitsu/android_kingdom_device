...
int tfa98xxSelectStereo(Tfa98xx_handle_t *handlesIn )
{
   Tfa98xx_Error_t err87;
...
static Tfa98xx_Error_t tfa98xxSetEqualizer(Tfa98xx_handle_t handle, const char* eqData) {
    ...
    while (eqData[i] != 0) {
        ...
        if ((b0 != 1) || (b1 != 0) || (b2 != 0) || (a1 != 0) || (a2 != 0)) {
        	err = tfa98xxCalcBiquadCoeff(b0, b1, b2, a1, a2, bytes);
        	PrintAssert( err);
        	err = Tfa98xx_DspBiquad_SetCoeffBytes(handle, ind, bytes);
            PrintAssert( err);
            if (err) return err;
        } else {
...
static Tfa98xx_Error_t tfa98xxSetEqualizerMultiple(int handle_cnt, Tfa98xx_handle_t *handlesIn, const char* eqData)
{
    int i;
    int ind;
    float b0, b1, b2, a1, a2;
...
nxpTfa98xx_Error_t nxpTfa98xxSetVolume( Tfa98xx_handle_t *handlesIn, int profile, int step ) // apply volume step, or mute
{
    ...
    tfa98xxReadRegister(0, handlesIn); // TODO what to check , will fail furtheron
...
int tfa98xxSetParamsMultiple(   nxpTfa98xxParamsType_t params, void *data, int length, 
                        Tfa98xx_handle_t handles[], int handle_cnt)
{
   Tfa98xx_Error_t err;
   int actualLength = 0;
   ...
   */
   ...
   {
      case tfa_speaker_params:
         err = Tfa98xx_DspWriteSpeakerParametersMultiple(handle_cnt, handles, length, (unsigned char *) data );
         PrintAssert(err);
         actualLength = err ? 0 : length;
         break;
      case tfa_config_params:
         err = Tfa98xx_DspWriteConfigMultiple(handle_cnt, handles, length, (const unsigned char *) data );
         PrintAssert(err);
         actualLength = err ? 0 :  length;
         break;
      case tfa_preset_params:
         err = Tfa98xx_DspWritePresetMultiple(handle_cnt, handles, length, ( unsigned char *) data );
         PrintAssert(err);
         actualLength = err ? 0 : length;
         break;
      case tfa_equalizer_params:
         err = tfa98xxSetEqualizerMultiple(handle_cnt, handles, (const char *) data);
         PrintAssert(err);
         actualLength = length;// TODO err ? 0 : Tfa98xx_BIQUAD_LENGTH;
         break;
      default:
 ...
 */
int tfa98xxGetCalibrationImpedance( FIXEDPT* re0, Tfa98xx_handle_t *handlesIn   )
{
    Tfa98xx_Error_t err;
...
float tfa98xxTcoef(float tCoef)
{
    ...
    current = tfa98xxCaltCoefFromSpeaker(tfaParams.speakerBuffer);

    if ( tCoef )
    	tfa98xxCaltCoefToSpeaker(tfaParams.speakerBuffer, tCoef);
...
int nxpTfa98xxCalibrationStereo(Tfa98xx_handle_t *handlesIn, int once)
{
  ...
  tCoef = tfa98xxCaltCoefFromSpeaker(tfaParams.speakerBuffer);
  ...
  for (i = 0; i < 2; i++)
  {
    ...
    printf("tCoef = %1.5f\n", tCoef);
    if(tfa98xxCalCheckMTPEX(handlesIn[i]) == 0)
    {
      ...
      if (!tfa98xxCalDspSupporttCoef(handlesIn[i]))
      {
        printf("still tCoef in speaker file, special calibration needed\n");

        err = tfa98xxCalComputeSpeakertCoefA(handlesIn[i], tfaParams.speakerBuffer, tCoef);
        if (err != Tfa98xx_Error_Ok)
        {
           return err;
        };
      ...
      err = tfaRun_MuteAmplifier(handlesIn[i]); /* clean shutdown to avoid plop */
        PrintAssert( err);
        tfaRun_ColdStartup(handlesIn[i]);
      }
    }
  ...
  for (i = 0; i < 2; i++)
  {
    ...
    printf("Calibration value is %2.2llu ohm\n", re25);
...
int nxptfa98xxbypassdsp(Tfa98xx_handle_t *handlesIn)
{
   ...
   err = Tfa98xx_ReadRegister16( handlesIn[idx], TFA98XX_BAT_PROT, &batProtRead);
   ...
   /* TODO: for 87, it's different MSK value */
   /* Only Sony currently using battery safeguard */
   batProtRead |= TFA989X_BAT_PROT_BSSBY_MSK;/* Bypass battery safeguard */
   ...
   err = Tfa98xx_WriteRegister16( handlesIn[idx], TFA98XX_BAT_PROT, batProtRead);
...
int nxptfa98xxunbypassdsp(Tfa98xx_handle_t *handlesIn)
{
   ...
   sysRead |= TFA9890_I2S_SEL_REG_POR;/* Set I2S SEL REG to set DCDC compensation to default 100%*/
   sysRead &= ~(TFA98XX_I2S_SEL_REG_SPKR_MSK);/*Set impedance to be defined by DSP */
   sysCtrlRead |= TFA98XX_SYS_CTRL_DCA_MSK;/* Set DCDC to active mode*/
   sysCtrlRead |= TFA98XX_SYS_CTRL_CFE_MSK;/* Enable Coolflux */
   /* TODO: for 87, it's different MSK value */
   /* Only Sony currently using battery safeguard */
   batProtRead &= ~(TFA989X_BAT_PROT_BSSBY_MSK);/* Enable battery safeguard */
 ...
 */
nxpTfa98xx_Error_t tfa98xxCalcBiquadCoeff(FIXEDPT b0, FIXEDPT b1, FIXEDPT b2,
                FIXEDPT a1, FIXEDPT a2, unsigned char bytes[BIQUAD_COEFF_SIZE * 3])
{
        FIXEDPT max_coeff;
        FIXEDPT mask;
        int headroom;
        ...
        coeff_buffer[1] = (int24) (-a2 * (1 << (23 - headroom)));
        coeff_buffer[2] = (int24) (-a1 * (1 << (23 - headroom)));
        coeff_buffer[3] = (int24) (b2 * (1 << (23 - headroom)));
        coeff_buffer[4] = (int24) (b1 * (1 << (23 - headroom)));
        coeff_buffer[5] = (int24) (b0 * (1 << (23 - headroom)));
/*
#ifdef __KERNEL__
